...
def _normalize(text: str) -> str:
    ...
    예: 'Ⅴ 영업이익' → '영업이익'
        'Ⅰ. 매출액'  → '매출액'
        '(2) 유형자산' → '유형자산'
        '대손충당금2,308,865,085106,...' → '대손충당금'  (뒤 숫자 제거)
    """
    # 로마숫자+아라비아숫자 접두사 제거 (예: Ⅴ, Ⅰ, Ⅹ, 1., (2), ①)
    t = re.sub(r'^[Ⅰ-Ⅹⅰ-ⅹ①-⑩\dⅠ]+[\s\.\)）]*', '', text.strip())
    # 공백·점·괄호 제거
    t = re.sub(r'[\s\u3000\u00a0()（）·・.]', '', t)
    # 한글/영문 과목명 뒤에 바로 붙은 숫자·콤마 제거
    # 예: '대손충당금2,308,865,085106,232,190,223' → '대손충당금'
    #     '감가상각누계액37,123,456' → '감가상각누계액'
    t = re.sub(r'(?<=[가-힣a-zA-Z])[\d,]+$', '', t)
    return t
